...
def add_student(conn: sqlite3.Connection, name: str, email: str) -> int:
    ...
    cursor = conn.execute("INSERT INTO students(name, email) VALUES(?,?);",
                          (name, email))
...
def add_assignment(conn: sqlite3.Connection, title: str, max_points: int) -> int:
    ...
    cursor = conn.execute("INSERT INTO assignments(title, max_points) VALUES(?,?);",
                          (title, max_points))
    return cursor.lastrowid
...
def record_grade(conn: sqlite3.Connection, student_id: int, assignment_id: int, score: int) -> int:
    ...
    cursor = conn.execute("INSERT INTO grades(student_id, assignment_id, score) VALUES(?,?,?);",
...
def student_grade_report(conn: sqlite3.Connection, student_id: int) -> list[sqlite3.Row]:
    ...
    """
    rows = """
             SELECT assignments.title AS assignment_title, score, max_points, 
             ROUND(1.0 * score / max_points * 100, 1) AS percent  
             FROM grades JOIN assignments on assignment_id = assignments.id
             WHERE grades.student_id = ?;
             """
    return conn.execute(rows, (student_id,)).fetchall()
def leaderboard(conn: sqlite3.Connection) -> list[sqlite3.Row]:
    ...
    """
    return conn.execute("""SELECT name AS student_name,   
                                  ROUND(AVG(1.0 * score / max_points * 100), 1) AS avg_percent  
                        FROM STUDENTS JOIN GRADES ON GRADES.student_id = STUDENTS.id 
                        JOIN ASSIGNMENTS ON ASSIGNMENTS.id = GRADES.assignment_id
                        Group by Students.id Order by avg_percent Desc;""").fetchall()
